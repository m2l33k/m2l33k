<!-- ===================== HACKER BANNER ===================== -->
<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f2027,50:203a43,100:00ff9c&height=220&section=header&text=m2l33k&fontColor=00ff9c&fontSize=85&animation=fadeIn&fontAlignY=38&desc=%3E%20Security%20Engineer%20//%20AI-driven%20Offensive%20Tooling%20//%20OWASP%20%2B%20Automation&descAlignY=60&descSize=17" width="100%"/>

<!-- glitch / matrix code stream -->
<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%"/>

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=24&pause=800&color=00FF9C&center=true&vCenter=true&width=780&height=90&lines=root%40m2l33k%3A~%23+access+granted+_;whoami+%3E+Malek+Aziz+Hassayoun;Security+Engineer+%2F%2F+Web+%26+AI+Pentesting;OWASP+ZAP+automation+%7C+anomaly+detection;Breaking+things+to+learn+how+they+work;sudo+hack+--the+--planet+--ethically" alt="Typing SVG" />
</a>

<br/>

<!-- ===================== SOCIAL / STATUS BADGES ===================== -->
<p>
  <a href="https://www.linkedin.com/in/malek-hassayoun/">
    <img alt="LinkedIn" src="https://img.shields.io/badge/LinkedIn-0A0A0A?style=for-the-badge&logo=linkedin&logoColor=00FF9C" />
  </a>
  <a href="https://instagram.com/m2l3k.arm">
    <img alt="Instagram" src="https://img.shields.io/badge/Instagram-0A0A0A?style=for-the-badge&logo=instagram&logoColor=00FF9C" />
  </a>
  <a href="mailto:hassayounmalekaziz@gmail.com">
    <img alt="Email" src="https://img.shields.io/badge/Email-0A0A0A?style=for-the-badge&logo=gmail&logoColor=00FF9C" />
  </a>
  <a href="https://facebook.com/malek.hassayoun">
    <img alt="Facebook" src="https://img.shields.io/badge/Facebook-0A0A0A?style=for-the-badge&logo=facebook&logoColor=00FF9C" />
  </a>
  <a href="https://medium.com/@malekaziz.hassayoun">
    <img alt="Medium" src="https://img.shields.io/badge/Medium-0A0A0A?style=for-the-badge&logo=medium&logoColor=00FF9C" />
  </a>
</p>

<p>
  <img alt="Profile views" src="https://komarev.com/ghpvc/?username=m2l33k&style=for-the-badge&color=00ff9c&label=INTRUSIONS" />
  <img alt="Followers" src="https://img.shields.io/github/followers/m2l33k?label=FOLLOWERS&style=for-the-badge&color=00ff9c&labelColor=0A0A0A" />
  <img alt="Stars" src="https://img.shields.io/github/stars/m2l33k?affiliations=OWNER%2CCOLLABORATOR&label=STARS&style=for-the-badge&color=00ff9c&labelColor=0A0A0A" />
</p>

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%"/>

</div>

<!-- ===================== ABOUT ===================== -->
```console
┌──(m2l33k㉿kali)-[~]
└─$ cat ./about.txt
```

## <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=20&pause=1000&color=00FF9C&width=200&height=32&lines=%3E+whoami" alt="whoami"/>

<img align="right" width="330" src="https://user-images.githubusercontent.com/74038190/212257467-871d32b7-e401-42e8-a166-fcfd7baa4c6b.gif" />

```python
class M2l33k:
    def __init__(self):
        self.name        = "Malek Aziz Hassayoun"
        self.role        = "IT Student | Volunteer | Aspiring Pentester"
        self.focus       = ["Ethical Hacking", "Web Security", "Automation"]
        self.location    = "Ariana, Tunisia"
        self.education   = "B.S. Software Engineering @ ESPRIT"

    def mission(self):
        return "Analyze vulns, automate defense, sharpen detection"
```

I build and learn in cybersecurity and software development. My focus is on practical security work: analyzing vulnerabilities, automating defensive workflows, and experimenting with tools that improve detection and response.

- 🔭 **Currently:** researching smart-grid security (blockchain + AI anomaly detection) @ ESPRIT
- 🌱 **Learning:** advanced web exploitation, cloud security, and detection engineering
- 🛰️ **Interests:** penetration testing, web security, automation, incident response, secure coding
- 🎓 **Studying:** B.S. Software Engineering @ ESPRIT (Ariana, Tunisia)
- 🤝 **Open to:** security internships, CTF teams, IEEE projects & open-source collaborations
- 📡 **Reach me:** `hassayounmalekaziz@gmail.com` • Discord: `m2l3ktn`

<br clear="right"/>

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%"/>

<!-- ===================== TECH STACK ===================== -->
```console
┌──(m2l33k㉿kali)-[~]
└─$ ./enum_stack.sh --show-all
```

## <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=20&pause=1000&color=00FF9C&width=220&height=32&lines=%3E+arsenal" alt="arsenal"/>

<div align="center">

**`[ LANGUAGES ]`**

<img alt="Python" src="https://img.shields.io/badge/Python-0A0A0A?style=for-the-badge&logo=python&logoColor=00FF9C" />
<img alt="JavaScript" src="https://img.shields.io/badge/JavaScript-0A0A0A?style=for-the-badge&logo=javascript&logoColor=00FF9C" />
<img alt="TypeScript" src="https://img.shields.io/badge/TypeScript-0A0A0A?style=for-the-badge&logo=typescript&logoColor=00FF9C" />
<img alt="PHP" src="https://img.shields.io/badge/PHP-0A0A0A?style=for-the-badge&logo=php&logoColor=00FF9C" />
<img alt="C" src="https://img.shields.io/badge/C-0A0A0A?style=for-the-badge&logo=c&logoColor=00FF9C" />
<img alt="Java" src="https://img.shields.io/badge/Java-0A0A0A?style=for-the-badge&logo=openjdk&logoColor=00FF9C" />
<img alt="Bash" src="https://img.shields.io/badge/Bash-0A0A0A?style=for-the-badge&logo=gnubash&logoColor=00FF9C" />

**`[ FRAMEWORKS ]`**

<img alt="Next.js" src="https://img.shields.io/badge/Next.js-0A0A0A?style=for-the-badge&logo=nextdotjs&logoColor=00FF9C" />
<img alt="FastAPI" src="https://img.shields.io/badge/FastAPI-0A0A0A?style=for-the-badge&logo=fastapi&logoColor=00FF9C" />
<img alt="Flask" src="https://img.shields.io/badge/Flask-0A0A0A?style=for-the-badge&logo=flask&logoColor=00FF9C" />
<img alt="Laravel" src="https://img.shields.io/badge/Laravel-0A0A0A?style=for-the-badge&logo=laravel&logoColor=00FF9C" />
<img alt="Node.js" src="https://img.shields.io/badge/Node.js-0A0A0A?style=for-the-badge&logo=nodedotjs&logoColor=00FF9C" />
<img alt="React" src="https://img.shields.io/badge/React-0A0A0A?style=for-the-badge&logo=react&logoColor=00FF9C" />
<img alt="Angular" src="https://img.shields.io/badge/Angular-0A0A0A?style=for-the-badge&logo=angular&logoColor=00FF9C" />

**`[ INFRA & TOOLS ]`**

<img alt="Tailwind CSS" src="https://img.shields.io/badge/Tailwind%20CSS-0A0A0A?style=for-the-badge&logo=tailwindcss&logoColor=00FF9C" />
<img alt="Supabase" src="https://img.shields.io/badge/Supabase-0A0A0A?style=for-the-badge&logo=supabase&logoColor=00FF9C" />
<img alt="PostgreSQL" src="https://img.shields.io/badge/PostgreSQL-0A0A0A?style=for-the-badge&logo=postgresql&logoColor=00FF9C" />
<img alt="MySQL" src="https://img.shields.io/badge/MySQL-0A0A0A?style=for-the-badge&logo=mysql&logoColor=00FF9C" />
<img alt="Docker" src="https://img.shields.io/badge/Docker-0A0A0A?style=for-the-badge&logo=docker&logoColor=00FF9C" />
<img alt="Kubernetes" src="https://img.shields.io/badge/Kubernetes-0A0A0A?style=for-the-badge&logo=kubernetes&logoColor=00FF9C" />
<img alt="Linux" src="https://img.shields.io/badge/Linux-0A0A0A?style=for-the-badge&logo=linux&logoColor=00FF9C" />
<img alt="Git" src="https://img.shields.io/badge/Git-0A0A0A?style=for-the-badge&logo=git&logoColor=00FF9C" />
<img alt="GitHub" src="https://img.shields.io/badge/GitHub-0A0A0A?style=for-the-badge&logo=github&logoColor=00FF9C" />
<img alt="VS Code" src="https://img.shields.io/badge/VS%20Code-0A0A0A?style=for-the-badge&logo=visualstudiocode&logoColor=00FF9C" />

</div>

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%"/>

<!-- ===================== EDUCATION ===================== -->
```console
┌──(m2l33k㉿kali)-[~]
└─$ cat /var/log/education.log
```

## <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=20&pause=1000&color=00FF9C&width=260&height=32&lines=%3E+education" alt="education"/>

| Degree | Institution | Location | Period | Status | Focus Areas |
| --- | --- | --- | --- | --- | --- |
| B.S. Software Engineering | The High Private School of Engineering and Technology (ESPRIT) | Ariana, Tunisia | 2023 – Present | In Progress | Cybersecurity, AI & ML, Blockchain, DevSecOps, Software Architecture |
| Preparatory Studies for Engineering | Faculty of Sciences Monastir | Monastir, Tunisia | 2020 – 2023 | Graduated | STEM foundation, CS fundamentals, engineering principles |

- `[+]` ESPRIT coursework: Software Architecture, Cybersecurity, AI & Machine Learning, Blockchain Technology, DevSecOps
- `[+]` ESPRIT highlights: multiple pentesting internships, active in IEEE projects
- `[+]` Preparatory coursework: Mathematics, Physics, Computer Science Fundamentals, Engineering Principles, Problem Solving

## <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=20&pause=1000&color=00FF9C&width=300&height=32&lines=%3E+online_courses" alt="online courses"/>

| Course | Provider | Year |
| --- | --- | --- |
| CyberSecurity 101 | TryHackMe | 2025 |
| Global Education Program Focused on AI | Samsung Innovation Campus | 2025 |

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%"/>

<!-- ===================== INTERNSHIPS ===================== -->
```console
┌──(m2l33k㉿kali)-[~]
└─$ ./recon.sh --target internships
```

## <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=20&pause=1000&color=00FF9C&width=280&height=32&lines=%3E+internships" alt="internships"/>

| Company | Role | Period | Location | Technologies | Key Outcome | Proof |
| --- | --- | --- | --- | --- | --- | --- |
| OXYNAS | Intern in AI-powered web application pentesting | Jun 2025 – Aug 2025 | Paris / Remote | AI, Web Security, Python, Automation | Built an AI pentesting agent to reduce manual testing time | [Certificate](https://res.cloudinary.com/dvmuvjukn/image/upload/v1766060184/Capture_d_%C3%A9cran_2025-12-18_131425_kcqq6t.png) |
| ESPRIT | Blockchain & AI research intern – smart grid security | Jul 2025 – Present | Tunis, Tunisia | Blockchain, AI, Smart Grid, Security Research | Benchmarked anomaly detection latency (49.5 ms) in simulation | [Certificate](https://res.cloudinary.com/dvmuvjukn/image/upload/v1766060218/WhatsApp_Image_2025-12-18_at_13.16.30_s57ofs.jpg) |
| TALAN-TUNISIA | Pentesting web application intern | Jun 2024 – Sep 2024 | Tunis, Tunisia | OWASP ZAP, Python, Web Security, Automation | Automated OWASP ZAP workflows with scripts | [Certificate](https://res.cloudinary.com/dvmuvjukn/image/upload/v1766060199/Capture_d_%C3%A9cran_2025-12-18_131434_qjcrmc.png) |
| ESPRIT | Pentesting WiFi/Bluetooth intern | Jun 2024 – Aug 2024 | Tunis, Tunisia | Wireless Security, Bluetooth Security, Network Attacks, Countermeasures | Reduced attack surface by 40% via countermeasures | [Certificate](https://res.cloudinary.com/dvmuvjukn/image/upload/v1766060203/Capture_d_%C3%A9cran_2025-12-18_131442_r92h0t.png) |

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%"/>

<!-- ===================== PROJECTS ===================== -->
```console
┌──(m2l33k㉿kali)-[~]
└─$ ls -la ./payloads/
```

## <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=20&pause=1000&color=00FF9C&width=240&height=32&lines=%3E+projects" alt="projects"/>

### `[ featured ]` — pinned exploits

<table>
<tr>
<td width="50%" valign="top">

#### 🛡️ [Cyber Sentinel — ASAT](https://github.com/m2l33k/ASAT)
Automated SAST + recon in one pipeline: Semgrep scanning, Nmap enumeration, and AI-assisted vulnerability assessment.

`Python` · `Semgrep` · `Nmap` · `AI/ML`

![Stars](https://img.shields.io/github/stars/m2l33k/ASAT?style=for-the-badge&color=00ff9c&labelColor=0A0A0A)
![Last Commit](https://img.shields.io/github/last-commit/m2l33k/ASAT?style=for-the-badge&color=00ff9c&labelColor=0A0A0A)

</td>
<td width="50%" valign="top">

#### 🚫 [Offensive Message Filter](https://github.com/m2l33k/filtre-chain)
Real-time ML classifier served over a REST API to flag toxic/abusive content at ingest.

`Python` · `ML` · `REST` · `Angular`

![Stars](https://img.shields.io/github/stars/m2l33k/filtre-chain?style=for-the-badge&color=00ff9c&labelColor=0A0A0A)
![Last Commit](https://img.shields.io/github/last-commit/m2l33k/filtre-chain?style=for-the-badge&color=00ff9c&labelColor=0A0A0A)

</td>
</tr>
<tr>
<td width="50%" valign="top">

#### ⚔️ [Code Arena AI — SentenX](https://github.com/m2l33k/SentenX)
Multi-agent coding tournament: model-vs-model battle arena with a human-in-the-loop review UI.

`Python` · `Flask` · `Ollama`

![Stars](https://img.shields.io/github/stars/m2l33k/SentenX?style=for-the-badge&color=00ff9c&labelColor=0A0A0A)
![Last Commit](https://img.shields.io/github/last-commit/m2l33k/SentenX?style=for-the-badge&color=00ff9c&labelColor=0A0A0A)

</td>
<td width="50%" valign="top">

#### 🧠 [Cognicare (IEEE)](https://github.com/m2l33k/ConiCare)
AI-driven assessment with gamified therapy and progress tracking.

`Next.js 14` · `FastAPI` · `Supabase` · `OpenCV`

![Stars](https://img.shields.io/github/stars/m2l33k/ConiCare?style=for-the-badge&color=00ff9c&labelColor=0A0A0A)
![Last Commit](https://img.shields.io/github/last-commit/m2l33k/ConiCare?style=for-the-badge&color=00ff9c&labelColor=0A0A0A)

</td>
</tr>
</table>

### `[ public_repos ]` — full archive

| Project | Period | Category | Status | Highlights | Stack (high level) | Repo Badges |
| --- | --- | --- | --- | --- | --- | --- |
| Cognicare (IEEE) | Dec 2025 | AI | Completed | AI assessment, gamified therapy, progress tracking | Next.js 14, FastAPI, Supabase, OpenCV, Tailwind | [Repo](https://github.com/m2l33k/ConiCare) ![Stars](https://img.shields.io/github/stars/m2l33k/ConiCare?style=flat) ![Issues](https://img.shields.io/github/issues/m2l33k/ConiCare?style=flat) ![Last Commit](https://img.shields.io/github/last-commit/m2l33k/ConiCare?style=flat) |
| ZenFlow Sanctuary | 2025 | Dev | Completed | Ambient mixer, Pomodoro, scene saving UI | React 18, TypeScript, Tailwind, Vite | [Repo](https://github.com/m2l33k/ZenFLow) ![Stars](https://img.shields.io/github/stars/m2l33k/ZenFLow?style=flat) ![Issues](https://img.shields.io/github/issues/m2l33k/ZenFLow?style=flat) ![Last Commit](https://img.shields.io/github/last-commit/m2l33k/ZenFLow?style=flat) |
| Code Arena AI: Multi-Agent Coding Tournament | Nov 2025 | AI | Completed | Multi-agent arena, battle UI, HITL | Python, Flask, Ollama, multi-model arena | [Repo](https://github.com/m2l33k/SentenX) ![Stars](https://img.shields.io/github/stars/m2l33k/SentenX?style=flat) ![Issues](https://img.shields.io/github/issues/m2l33k/SentenX?style=flat) ![Last Commit](https://img.shields.io/github/last-commit/m2l33k/SentenX?style=flat) |
| Decentralized Swarm Logistics Simulation | Sep 2025 | AI | In Development | Swarm bidding, PoA chain, LLM oversight | Python, Ollama, numpy, pygame | [Repo](https://github.com/m2l33k/Strategy-Aware) ![Stars](https://img.shields.io/github/stars/m2l33k/Strategy-Aware?style=flat) ![Issues](https://img.shields.io/github/issues/m2l33k/Strategy-Aware?style=flat) ![Last Commit](https://img.shields.io/github/last-commit/m2l33k/Strategy-Aware?style=flat) |
| Offensive Message Filter System | 2024 | AI | Completed | Real-time filtering, ML classifier, REST API | Python, ML, REST API, Angular | [Repo](https://github.com/m2l33k/filtre-chain) ![Stars](https://img.shields.io/github/stars/m2l33k/filtre-chain?style=flat) ![Issues](https://img.shields.io/github/issues/m2l33k/filtre-chain?style=flat) ![Last Commit](https://img.shields.io/github/last-commit/m2l33k/filtre-chain?style=flat) |
| Cyber Sentinel – Advanced Security Analysis Tool | 2024 | Cybersecurity | Completed | Semgrep scans, Nmap recon, AI assessment | Python, Semgrep, Nmap, AI/ML | [Repo](https://github.com/m2l33k/ASAT) ![Stars](https://img.shields.io/github/stars/m2l33k/ASAT?style=flat) ![Issues](https://img.shields.io/github/issues/m2l33k/ASAT?style=flat) ![Last Commit](https://img.shields.io/github/last-commit/m2l33k/ASAT?style=flat) |
| TechPioneers | 2024 | Dev | Completed | E-learning platform, modern SWE focus | Symfony 6, MySQL, JS | [Repo](https://github.com/m2l33k/techPioneers) ![Stars](https://img.shields.io/github/stars/m2l33k/techPioneers?style=flat) ![Issues](https://img.shields.io/github/issues/m2l33k/techPioneers?style=flat) ![Last Commit](https://img.shields.io/github/last-commit/m2l33k/techPioneers?style=flat) |
| Luducatio (Chess Robot) | 2024 | AI | Completed | Chess learning robot, CV + ML | Python, ML, OpenCV | [Repo](https://github.com/m2l33k/ChessRobot) ![Stars](https://img.shields.io/github/stars/m2l33k/ChessRobot?style=flat) ![Issues](https://img.shields.io/github/issues/m2l33k/ChessRobot?style=flat) ![Last Commit](https://img.shields.io/github/last-commit/m2l33k/ChessRobot?style=flat) |

### `[ other_work ]` — no public repo linked

| Project | Category | Period | Outcome |
| --- | --- | --- | --- |
| ERP system for ISO 9001 process automation (Cocosunult) | Dev | Oct 2024 – May 2025 | Automated ISO 9001 compliance, reduced manual work by 60% |
| AI-driven cybersecurity incident response platform (IEEE) | AI | Jul 2024 – Sep 2024 | Accelerated response by 35% with Snort/Suricata integration |
| Basic network sniffer | Cybersecurity | 2024 | Live packet capture & protocol analysis (TCP/UDP/ICMP) with Scapy to flag anomalous traffic patterns |
| Phishing awareness training | Cybersecurity | 2024 | Interactive simulation + training module to teach users to spot and report phishing attempts |
| NextJEEL | Dev | 2024 | Modern responsive platform for interactive learning |

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%"/>

<!-- ===================== CERTIFICATIONS ===================== -->
```console
┌──(m2l33k㉿kali)-[~]
└─$ gpg --verify ./certs/*.sig
```

## <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=20&pause=1000&color=00FF9C&width=320&height=32&lines=%3E+certifications" alt="certifications"/>

### `[ featured — security ]`

| Certification | Issuer | Year | Credential |
| --- | --- | --- | --- |
| **Ethical Hacker** | Cisco | 2025 | [Verify](https://www.credly.com/badges/d99c8f70-d850-4db5-a0f8-8f95d57b6623/public_url) |
| **Junior Cybersecurity Analyst** | Cisco | 2025 | [Verify](https://www.credly.com/badges/213294e0-c258-4c39-acda-2036a43982bf/public_url) |
| **AppSec (CAP)** | The SECOPS GROUP | 2025 | ID `10087776` |
| **Cisco Cybersecurity** | Cisco | 2024 | [Verify](https://www.credly.com/badges/a48a3270-e587-47a0-ad30-9a183c5c4c42/public_url) |
| **Networking Basics** | Cisco | 2025 | [Verify](https://www.credly.com/badges/9fd70672-a3d4-4f9e-965b-a0e595643164/public_url) |
| **TryHackMe Pre Security** | TryHackMe | 2025 | [Verify](https://tryhackme-certificates.s3-eu-west-1.amazonaws.com/THM-YALUUJOW4H.pdf) |

<details>
<summary><b>📜 View all 23 certifications (AI · Cybersecurity · Dev)</b></summary>

<br/>

| Certification | Issuer | Year | Category | Status | Credential ID | Link |
| --- | --- | --- | --- | --- | --- | --- |
| Fundamentals of Deep Learning | NVIDIA | 2025 | AI | Active | 1tO0Ys3ITkGJkXM3sgBKrQ | https://learn.nvidia.com/certificates?id=1tO0Ys3ITkGJkXM3sgBKrQ |
| Evaluation and Light Customization of Large Language Models | NVIDIA | 2025 | AI | Active | M9kdR7yQQ5WiuvosdW_R4g | https://learn.nvidia.com/certificates?id=M9kdR7yQQ5WiuvosdW_R4g |
| Building RAG Agents with LLMs | NVIDIA | 2025 | AI | Active | qjskPdObSiC6MS5RlXreLA | https://learn.nvidia.com/certificates?id=qjskPdObSiC6MS5RlXreLA |
| Getting Started with AI on Jetson Nano | NVIDIA | 2025 | AI | Active | GnzRATHpSIajte8xXpnjoA | https://learn.nvidia.com/certificates?id=GnzRATHpSIajte8xXpnjoA |
| AI for Anomaly Detection | NVIDIA | 2025 | AI | Active | TDSSTGVqTMmdSBKhU66_bg | https://learn.nvidia.com/certificates?id=TDSSTGVqTMmdSBKhU66_bg |
| Generative AI with Diffusion Models | NVIDIA | 2025 | AI | Active | PdQJK76xS2OHwU0Yn_AIWA | https://learn.nvidia.com/certificates?id=PdQJK76xS2OHwU0Yn_AIWA |
| AI for Predictive Maintenance | NVIDIA | 2025 | AI | Active | HTAHASfdRuekTxCPxk5QfA | https://learn.nvidia.com/certificates?id=HTAHASfdRuekTxCPxk5QfA# |
| Natural Language Processing | NVIDIA | 2025 | AI | Active | 9Pfvt0O-Sz29etyyFrXRFw | https://learn.nvidia.com/certificates?id=9Pfvt0O-Sz29etyyFrXRFw |
| Oracle AI Vector Search Certified Professional | Oracle | 2025 | AI | Active | 3E1FD1803CC8A032863018AAA3A21D25C6D09345062A6BFB2848FF12D7C94AAE | https://catalog-education.oracle.com/pls/certview/sharebadge?id=3E1FD1803CC8A032863018AAA3A21D25C6D09345062A6BFB2848FF12D7C94AAE |
| AppSec (CAP) | The SECOPS GROUP | 2025 | Cybersecurity | Active | 10087776 | https://pentestingexams.com/certificate-validation/ |
| TryHackMe Pre Security | TryHackMe | 2025 | Cybersecurity | Active | THM-PRESEC-001 | https://tryhackme-certificates.s3-eu-west-1.amazonaws.com/THM-YALUUJOW4H.pdf |
| DevSecOps Learning | TryHackMe | 2025 | Cybersecurity | Active | DEVSECOPS-001 | https://tryhackme-certificates.s3-eu-west-1.amazonaws.com/THM-YALUUJOW4H.pdf |
| Cisco Cybersecurity | Cisco | 2024 | Cybersecurity | Active | a48a3270-e587-47a0-ad30-9a183c5c4c42 | https://www.credly.com/badges/a48a3270-e587-47a0-ad30-9a183c5c4c42/public_url |
| Ethical Hacker | Cisco | 2025 | Cybersecurity | Active | d99c8f70-d850-4db5-a0f8-8f95d57b6623 | https://www.credly.com/badges/d99c8f70-d850-4db5-a0f8-8f95d57b6623/public_url |
| Networking Basics | Cisco | 2025 | Cybersecurity | Active | 9fd70672-a3d4-4f9e-965b-a0e595643164 | https://www.credly.com/badges/9fd70672-a3d4-4f9e-965b-a0e595643164/public_url |
| Junior Cybersecurity Analyst | Cisco | 2025 | Cybersecurity | Active | 213294e0-c258-4c39-acda-2036a43982bf | https://www.credly.com/badges/213294e0-c258-4c39-acda-2036a43982bf/public_url |
| Scrum Foundation | CertiProf | 2024 | Dev | Active | 8440a79c-603f-485b-9f98-8fca213a817c | https://www.credly.com/badges/8440a79c-603f-485b-9f98-8fca213a817c/public_url |
| Hashgraph Developer | The Hashgraph Association | 2025 | Dev | Active | 2d0ed167-0b26-41b3-a16d-5d373cd4463b | https://certs.hashgraphdev.com/2d0ed167-0b26-41b3-a16d-5d373cd4463b.pdf |
| Lifelong Learning 2025 | CertiProf | 2025 | Dev | Active | 1f5ae449-0f69-4587-b700-a41ebd54178c | https://www.credly.com/badges/1f5ae449-0f69-4587-b700-a41ebd54178c/public_url |
| Github Foundation | GitHub | 2025 | Dev | Active | 2307d695-4e5c-4488-8578-7cda57c82d0a | https://www.credly.com/badges/2307d695-4e5c-4488-8578-7cda57c82d0a/public_url |
| IBM Data Visualization | IBM | 2024 | Dev | Active | 3e8597df-085b-4c34-be6a-a47a657f93b6 | https://www.credly.com/badges/3e8597df-085b-4c34-be6a-a47a657f93b6/public_url |
| Python Essentials 1 | Python Institute | 2025 | Dev | Active | 054bf6b2-8d3d-468e-a078-ec3473a2afff | https://www.credly.com/badges/054bf6b2-8d3d-468e-a078-ec3473a2afff/public_url |
| AWS Training Badge | AWS | 2025 | Dev | Active | c8c788e5-d0d9-4723-95b4-e78d4c268aec | https://www.credly.com/badges/c8c788e5-d0d9-4723-95b4-e78d4c268aec/public_url |

</details>

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%"/>

<!-- ===================== WRITEUPS ===================== -->
```console
┌──(m2l33k㉿kali)-[~]
└─$ tail -f ./writeups/latest.log
```

## <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=20&pause=1000&color=00FF9C&width=240&height=32&lines=%3E+writeups" alt="writeups"/>

Security notes, CTF writeups, and research I publish. Full archive on [Medium →](https://medium.com/@malekaziz.hassayoun)

<!-- Add your latest posts here as: | Title | Topic | Link | -->
| Post | Topic | Link |
| --- | --- | --- |
| _First writeup coming soon…_ | Web / Pentest | [Medium](https://medium.com/@malekaziz.hassayoun) |

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%"/>

<!-- ===================== GITHUB STATS ===================== -->
```console
┌──(m2l33k㉿kali)-[~]
└─$ ./exfil_stats.sh --format matrix
```

## <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=20&pause=1000&color=00FF9C&width=300&height=32&lines=%3E+github_stats" alt="github stats"/>

<div align="center">

<!-- Stats + languages + habits, generated by .github/workflows/metrics.yml (committed SVG, no rate limits) -->
<img alt="GitHub Metrics" src="https://raw.githubusercontent.com/m2l33k/m2l33k/main/metrics.terminal.svg" />

<br/><br/>

<img alt="GitHub Streak" height="180" src="https://streak-stats.demolab.com?user=m2l33k&theme=chartreuse-dark&hide_border=true&background=0A0A0A&ring=00FF9C&fire=00FF9C&currStreakLabel=00FF9C" />

<br/><br/>

<!-- Achievements (trophy replacement), generated by .github/workflows/metrics.yml -->
<img alt="Achievements" src="https://raw.githubusercontent.com/m2l33k/m2l33k/main/metrics.achievements.svg" />

<br/><br/>

<!-- Contribution snake (auto-generated by .github/workflows/snake.yml on the 'output' branch) -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/m2l33k/m2l33k/output/snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/m2l33k/m2l33k/output/snake.svg" />
  <img alt="snake animation" src="https://raw.githubusercontent.com/m2l33k/m2l33k/output/snake.svg" />
</picture>

<br/><br/>

<img alt="Activity Graph" src="https://github-readme-activity-graph.vercel.app/graph?username=m2l33k&theme=high-contrast&bg_color=0A0A0A&color=00FF9C&line=00FF9C&point=FFFFFF&hide_border=true" />

<br/>

<img alt="Visitor counter" src="https://count.getloli.com/get/@m2l33k?theme=moebooru" />
<br/><br/>
<img alt="Visitors" src="https://visitor-badge.laobi.icu/badge?page_id=m2l33k.m2l33k&left_text=VISITORS&left_color=0A0A0A&right_color=00ff9c" />

</div>

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%"/>

<!-- ===================== FOOTER ===================== -->
<div align="center">

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=20&pause=1000&color=00FF9C&center=true&vCenter=true&width=620&lines=Thanks+for+visiting+my+node.;Stay+curious.+Hack+ethically.;%5B+connection+closed+%5D" alt="footer typing" />
</a>

**`>` Open to security internships & collaborations — let's connect.**

<a href="mailto:hassayounmalekaziz@gmail.com">
  <img alt="Email" src="https://img.shields.io/badge/Reach%20out-0A0A0A?style=for-the-badge&logo=gmail&logoColor=00FF9C" />
</a>
<a href="https://www.linkedin.com/in/malek-hassayoun/">
  <img alt="LinkedIn" src="https://img.shields.io/badge/Connect-0A0A0A?style=for-the-badge&logo=linkedin&logoColor=00FF9C" />
</a>

```console
┌──(m2l33k㉿kali)-[~]
└─$ exit
[+] Connection closed. Stay curious. Hack ethically.
```

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00ff9c,50:203a43,100:0f2027&height=140&section=footer" width="100%"/>

</div>
